...
// 종속 드롭다운 — 마스터 데이터 시트 + C/D/E 컬럼 매핑
const MASTER_SHEET_NAME = '마스터 데이터';
const TEAM_COL = 3;          // C — 팀
const MANAGER_COL = 4;       // D — 담당자
const CLIENT_COL = 5;        // E — 광고주
const DROPDOWN_END_ROW = 1000;
...
function onEditTrigger(e) {
  ...
  // [종속 드롭다운] 신규·유지보수 시트의 C/D/E 변경 시 동적 검증 갱신 (v0.2.8)
  // 자체 try/catch 갖고 있어서 실패해도 아래 ID 발급·캘린더 등록 흐름은 정상 진행.
  try {
    handleCascadingDropdown_(e);
  } catch (err) {
    log_('onEditTrigger', `cascading dropdown 에러: ${err}\n${err.stack || ''}`);
  }
...
// ============================================================
// 종속 드롭다운 (Cascading Dropdown) — v0.2.8 GAS 확장
// C(팀) ↔ D(담당자) ↔ E(광고주) 동적 검증.
//
// 마스터 데이터 시트('마스터 데이터') 헤더 패턴으로 자동 매핑:
//   2행 헤더 — A='팀', '{팀명} 담당자', '{팀명} 광고주'
//   3행~    — 데이터
// 팀 추가 시 마스터 시트에 컬럼만 추가하면 코드 수정 없이 자동 반영.
...
/**
 * 마스터 데이터 시트 읽기 + 가공 + 5분 캐시.
 * 반환:
 *   { teams, managersByTeam, clientsByTeam, teamByManager, allManagers, allClients }
 *   시트 없거나 데이터 없으면 null.
 */
function getMasterData_() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('master_data_v1');
  if (cached) return JSON.parse(cached);

  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(MASTER_SHEET_NAME);
  if (!sheet) {
    log_('getMasterData_', `'${MASTER_SHEET_NAME}' 시트 없음 — null`);
    return null;
  }
  const lastCol = sheet.getLastColumn();
  const lastRow = sheet.getLastRow();
  if (lastRow < 3 || lastCol < 1) {
    log_('getMasterData_', `데이터 없음 (lastRow=${lastRow}, lastCol=${lastCol})`);
    return null;
  }
  ...
  const teamColIdx = headers.findIndex(h => String(h || '').trim() === '팀');
  const managerCols = {}; // { 팀명: 컬럼 인덱스 }
  const clientCols = {};
  headers.forEach((h, idx) => {
    const header = String(h || '').trim();
    const mManager = header.match(/^(.+)\s담당자$/);
    const mClient = header.match(/^(.+)\s광고주$/);
    if (mManager) managerCols[mManager[1].trim()] = idx;
    else if (mClient) clientCols[mClient[1].trim()] = idx;
  });

  const teams = teamColIdx === -1 ? [] :
    values.map(r => String(r[teamColIdx] || '').trim()).filter(v => v);

  const managersByTeam = {};
  for (const team in managerCols) {
    managersByTeam[team] = values
      .map(r => String(r[managerCols[team]] || '').trim())
      .filter(v => v);
  }
  const clientsByTeam = {};
  for (const team in clientCols) {
    clientsByTeam[team] = values
      .map(r => String(r[clientCols[team]] || '').trim())
      .filter(v => v);
  }

  // 담당자 → 팀 역방향 매핑 (D 자동 채우기용). 1:1 가정.
  const teamByManager = {};
  for (const team in managersByTeam) {
    managersByTeam[team].forEach(m => { teamByManager[m] = team; });
  }

  const allManagers = [].concat(...Object.values(managersByTeam));
  const allClients = [].concat(...Object.values(clientsByTeam));

  const data = { teams, managersByTeam, clientsByTeam, teamByManager, allManagers, allClients };
  cache.put('master_data_v1', JSON.stringify(data), 300); // 5분
  log_('getMasterData_', `cache miss → fetch 완료 (팀 ${teams.length}, 담당자 ${allManagers.length}, 광고주 ${allClients.length})`);
  return data;
}

/**
 * Data Validation 헬퍼.
 * values 비어있으면 검증 제거(자유 입력 허용).
 * 잘못된 입력은 경고만(setAllowInvalid=true).
 */
...
/**
 * onEdit 분기 — 종속 드롭다운 처리.
 * 신규·유지보수 시트 + C/D/E 데이터 영역만. 그 외 silent skip.
 */
function handleCascadingDropdown_(e) {
  if (!e) return;
  const sheet = e.range.getSheet();
  const sheetName = sheet.getName();
  if (!sheetName.includes('신규') || !sheetName.includes('유지보수')) return;
  ...
  const col = e.range.getColumn();
  if (col !== TEAM_COL && col !== MANAGER_COL && col !== CLIENT_COL) return;

  const master = getMasterData_();
  if (!master) {
    log_('handleCascadingDropdown_', `마스터 없음 — skip (row=${row}, col=${col})`);
    return;
  }
  ...
  if (col === TEAM_COL) {
    ...
    if (!newValue) return;
    const currentTeam = String(sheet.getRange(row, TEAM_COL).getValue() || '').trim();
    if (currentTeam) return; // C 이미 차있음 = 정방향. 추가 처리 X
    const inferredTeam = master.teamByManager[newValue];
    if (!inferredTeam) {
      log_('handleCascadingDropdown_', `D${row} '${newValue}' → 마스터에 없음. C 자동 채우기 skip`);
      return;
    }
    log_('handleCascadingDropdown_', `D${row} '${newValue}' → C에 '${inferredTeam}' 자동 채우기 + E 검증 갱신`);
    sheet.getRange(row, TEAM_COL).setValue(inferredTeam);
    setDropdown_(sheet.getRange(row, CLIENT_COL), master.clientsByTeam[inferredTeam] || []);
  }
  // col === CLIENT_COL: 자동 채우기 없음. silent.
}
...
function initCascadingDropdowns() {
  ...
  try {
    ...
    const master = getMasterData_();
    if (!master) {
      log_('initCascadingDropdowns', `마스터 데이터 없음 — 중단`);
      return;
    }

...
#### 현재 트리거 구성 (2026-08-05 기준)
| 트리거 | 주기 | 함수 |
|---|---|---|
| ON_EDIT / SPREADSHEETS | 편집 시 | `moveRowOnCheck` (공유 체크 → 완료 이관) |
...
- **간이 트리거는 꺼둠**: 2026-08-04 사고 때 투입해 업무를 복구했으나, 다음날 설치형이 복구되면서 한 편집에 이관이 2번 도는 상태가 됨. 트리거 실행시간을 2배로 태우고(개인 계정 하루 90분 한도) 로그도 지저분해져서 비활성화
  - **되살리는 법**: `onEditSimpleBackup_` → `onEdit`으로 이름 변경. 그게 전부
  - 되살릴 상황: 설치형 onEdit이 또 죽었는데 즉시 반응이 꼭 필요할 때. (업무 자체는 5분 트리거로 이미 보장되므로 급하지 않음)
- **`warmHolidayCache_`**: 설치형 시간 트리거(`processCheckedRowsNow` / `syncToCalendar`) 시작 시 공휴일 캐시를 미리 채움
  ...
  - 간이 트리거를 끈 지금은 필수는 아니지만, 되살릴 때를 위해 유지. 캐시 적중 시 즉시 반환이라 부하 없음
  - **⚠ 불필요한 호출로 보고 제거하지 말 것** — 간이 트리거 부활 시 TAT가 조용히 틀어진다
...
- **설치형 onEdit 트리거 재발 여부** — 08-04에 하루 죽었다 저절로 복구됨. 또 침묵하면 `onEditSimpleBackup_` → `onEdit` 이름 변경으로 즉시 우회 가능
- **공유 체크 이관** — 설치형 onEdit + 5분 시간 트리거 이중 경로가 안착하는지. 누락·중복 사례 모니터링
...
### ✅ 설치형 onEdit 트리거 하루 사망 (2026-08-04 ~ 08-05) — 자연 복구
- **증상**: M열 공유 체크가 무반응. 설치형 onEdit 트리거가 전부 발사되지 않음. **실행 기록조차 안 남음.** 진단용 최소 함수 `pingOnEdit`을 트리거로 걸어도 무반응. UI에서 지우고 다시 만들어도 동일.
- **범위 특정 — 여기가 진단의 핵심**:
  - `doGet`(웹앱) 정상 → 스크립트 코드·시트 바인딩·권한 멀쩡
  - **설치형 '시간' 트리거 `syncToCalendar`는 정상 동작** (2.995초 완료) → 계정 인증·할당량·구글 정책 문제가 아님. 그것들이면 시간 트리거도 같이 죽었을 것
  - 간이 트리거는 정상 발사 → 구글이 이 시트의 편집 이벤트를 감지는 하고 있음
  - **결론: "설치형 + 편집 이벤트" 조합만 배달 안 됨.** 코드로 설명 가능한 영역 밖
- **`repairInstallableEditTriggers`로 확인한 것**: 등록된 트리거 6개 전부 정상. 좀비·중복·고아 없음. `moveRowOnCheck`/`onEditTrigger`가 `ON_EDIT / SPREADSHEETS`로 **정확히 잘 박혀 있는데도 발사만 안 됐음** → "등록 손상" 가설도 폐기
- **복구**: 다음날 아침 **아무 조치 없이 저절로 정상화**. 구글 측 일시 장애로 판단
- **선행 사고와의 관계**: 직전에 `moveRowOnCheck` 8분 행(위 항목)이 반복되면서 트리거가 자동 비활성화된 것이 발단으로 추정되나, 재등록 후에도 안 살아난 이유는 끝내 미확정
- **교훈**: 트리거가 침묵할 때 **"유형" 열로 웹앱 / 시간 기반 / 편집 기반을 갈라볼 것.** 어느 조합이 살아있는지가 원인 후보를 한 번에 절반 이하로 줄인다

...
- **간이 트리거의 대가 (알고 쓸 것)**:
  | 항목 | 내용 |
  |---|---|
  | 권한 서비스 | `CalendarApp` 등 사용 불가 → TAT 공휴일 반영에 영향 (아래 캐시 데우기로 보정) |
  | 실패 알림 | **구글 "Summary of failures" 메일이 안 옴.** 설치형에만 발송 — 다음 사고 진단이 어려워짐 |
  | 실행 시간 | 30초 (설치형은 6분). 현재 1~2초라 여유는 있음 |
  | 실행 주체 | 편집한 팀원 계정으로 실행 (설치형은 소유자). 실행 로그 추적이 흐려질 수 있음 |
  | on/off | 트리거 UI에서 관리 불가. 함수명이 `onEdit`이면 무조건 동작, 끄려면 코드 수정 |
- **`warmHolidayCache_` — 위 첫 줄의 보정책**:
  - 간이 트리거는 `CalendarApp`을 못 써서, 공휴일 캐시가 비어 있으면 **주말만 제외한 TAT**가 기록된다. 같은 작업도 체크 시점에 따라 값이 달라지는 문제
  - 설치형 시간 트리거(`processCheckedRowsNow` 5분 / `syncToCalendar` 1시간)가 시작할 때 공휴일 캐시를 미리 채워둠. **캐시 TTL 6시간 > 트리거 주기**라 비는 구간이 사실상 없음
  - `calcBusinessDays`가 참조하는 연도는 올해·작년 둘뿐이라 그 두 개만 데움. 회사 휴무일은 바인딩 시트에서 읽으므로 간이 트리거도 직접 접근 가능 → 제외
  - **⚠ 이 호출을 지우면 TAT가 조용히 틀어진다.** 성능 최적화로 보고 제거하지 말 것

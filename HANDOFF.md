...
| `repairInstallableEditTriggers` | 수동 ▶ | 설치형 onEdit 트리거 전수 출력 → ON_EDIT만 삭제 → 코드로 재생성 |
| `onEditSimpleBackup_` | **비활성** | 간이 트리거. 이름을 `onEdit`으로 바꾸면 즉시 부활 |
| `pingOnEdit` / `whichSpreadsheetAmIBoundTo` | 진단용 | 트리거 생사 확인 / 바인딩된 시트 확인 |
| `sortCompleteSheet` | 수동 ▶ | 완료 시트 정렬 |

#### 현재 트리거 구성 (2026-08-24 기준)
| 트리거 | 주기 | 함수 |
|---|---|---|
| ON_EDIT / SPREADSHEETS (설치형) | 편집 시 | `moveRowOnCheck` (공유 체크 → 완료 이관) |
| ON_EDIT / SPREADSHEETS (설치형) | 편집 시 | `onEditTrigger` (L열 ID 발급·중복 해소) |
| onEdit (간이, 코드에 등록 없이 자동) | 편집 시 | 위 둘을 호출 — 상시 이중화, 08-24부터 활성 |
...
- **공유 체크 이관은 3중 경로**: 설치형 onEdit(즉시) + **간이 트리거 onEdit(즉시, 상시 이중화)** + 5분 시간 트리거(누락 회수)
  - 5분 트리거가 반드시 필요한 이유: **onEdit은 드래그로 여러 셀 채우기·붙여넣기를 못 잡는다.** 체크박스를 드래그로 여러 개 켜면 누락됨. 이건 간이든 설치형이든 동일한 한계
  - 경로가 겹쳐도 중복 이관 없음 — `moveRowOnCheck`가 lock 획득 후 M열 TRUE를 재확인
- **간이 트리거 운영 이력 — 상시 이중화로 정책 전환됨**:
  - 08-04: 설치형 onEdit 사망 시 최초 투입, 업무 복구
  - 08-05: 다음날 설치형이 저절로 복구 → 중복 실행 낭비 줄이려 `onEdit` → `onEditSimpleBackup_`로 이름 바꿔 비활성화
  - **08-24: 3주 만에 설치형이 또 침묵 (같은 좁은 범위 — 시간 트리거는 정상, onEdit 배달만 끊김).** 일회성이 아니라 간헐적 재발 패턴으로 판단 → `onEditSimpleBackup_` → `onEdit`으로 되돌려 **상시 켜둠**으로 정책 전환
  - **현재 상태: 간이 트리거 `onEdit` 활성.** 다시 재우려면 이름을 `onEditSimpleBackup_`으로 바꾸면 됨 (그럴 이유는 딱히 없음 — 중복 실행 비용보다 즉시성 확보가 낫다고 판단해서 유지하는 것)
- **`warmHolidayCache_`**: 설치형 시간 트리거(`processCheckedRowsNow` / `syncToCalendar`) 시작 시 공휴일 캐시를 미리 채움
  - 간이 트리거는 `CalendarApp`을 못 써서, 캐시가 비면 **주말만 제외한 TAT**가 기록됨 (같은 작업도 체크 시점에 따라 값이 달라짐). 그 대비책
  - 간이 트리거가 상시 켜져 있는 지금 **필수** — 제거하면 TAT가 조용히 틀어진다. 캐시 적중 시 즉시 반환이라 부하는 없음
...
- **설치형 onEdit 트리거 3차 재발 여부** — 08-04, 08-24 두 번 침묵. 08-24부터 간이 트리거 상시 이중화로 대응 중(알려진 이슈 참조). 또 재발해도 업무 영향 없어야 정상 — 실제로 그런지 관찰
- **공유 체크 이관** — 설치형 onEdit + 간이 onEdit + 5분 시간 트리거 3중 경로가 안착하는지. 누락·중복 사례 모니터링
...
### 설치형 onEdit 트리거 간헐적 침묵 (2026-08~, 재발성 — 상시 완화 적용 중)
- **증상**: M열 공유 체크가 무반응. 설치형 onEdit 트리거(`moveRowOnCheck`/`onEditTrigger`)가 발사 안 됨. 시간 트리거는 정상.
- **발생 이력**:
  - **1차 (08-04~08-05)**: 진단용 `pingOnEdit`조차 무반응, UI에서 지우고 다시 만들어도 동일. 다음날 아침 아무 조치 없이 자연 복구
  - **2차 (08-24)**: 오전 11:34까지 정상 발사되다 11:50경 재차 침묵. 5분 시간 트리거(`processCheckedRowsNow`)가 최대 5분 지연으로 계속 받쳐줘서 업무 영향은 없었음
- **범위 특정 — 두 번 다 동일한 패턴**:
  - `doGet`(웹앱) 정상 → 스크립트 코드·시트 바인딩·권한 멀쩡
  - **설치형 '시간' 트리거는 정상 동작** → 계정 인증·할당량·구글 정책 문제가 아님. 그것들이면 시간 트리거도 같이 죽었을 것
  - **결론: "설치형 + 편집 이벤트" 조합만 배달 안 됨.** 코드로 설명 가능한 영역 밖
- **`repairInstallableEditTriggers`(1차 때)로 확인한 것**: 등록된 트리거 전수 정상, 좀비·중복·고아 없음. `ON_EDIT / SPREADSHEETS`로 정확히 등록돼 있는데도 발사만 안 됐음 → "등록 손상" 가설 폐기
- **원인**: 끝내 미확정. 구글 쪽 onEdit 배달 계층의 간헐적 결함으로 추정
- **현재 대응 — 08-24부터 상시 이중화로 정책 전환**:
  - 08-05엔 "일회성"으로 보고 간이 트리거를 꺼뒀으나, 3주 내 재발로 "간헐적으로 재발하는 패턴"으로 재평가
  - **간이 트리거 `onEdit`을 상시 켜둠** (설치형과 별개 경로, 등록·승인 불필요) — 설치형이 죽어도 즉시 반응 유지
  - 5분 시간 트리거(`processCheckedRowsNow`)는 그 아래 최후 안전망으로 계속 유지 (onEdit이 못 잡는 드래그·붙여넣기 케이스 커버)
  - 셋이 겹쳐도 중복 이관 없음 — `moveRowOnCheck`가 lock 획득 후 M열 TRUE 재확인
- **교훈**: 트리거가 침묵할 때 **"유형" 열로 웹앱 / 시간 기반 / 편집 기반을 갈라볼 것.** 어느 조합이 살아있는지가 원인 후보를 한 번에 절반 이하로 줄인다. 그리고 재발하는 인프라 결함은 원인 규명보다 **상시 이중화가 정답**이다
